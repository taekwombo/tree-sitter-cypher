...
module.exports = grammar({
    ...
    rules: {
        ...
        // identifier_start: ($) => choice($.id_start, $.pc),
        // identifier_part: ($) => choice($.id_continue, $.sc),
        // id_start: ($) => /a-z/,
        // id_continue: ($) => /a-z/,
        // pc: ($) => /a-z/,
        // sc: ($) => /a-z/,
...
function word(w) {
    return alias(token(seq(
        ...w.split('').map((char) => choice(char.toLowerCase(), char.toUpperCase())),
    )), w);
    return new RegExp(w, 'i');
}

function expression(precedence, rule, assoc = 'left') {
