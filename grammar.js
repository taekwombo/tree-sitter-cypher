...
module.exports = grammar({
    ...
    rules: {
        ...
        expression: ($) => choice($.or_expression, $.xor_expression, $.and_expression, $.not_expression, $.comparison_expression, $.addition_expression, $.multiplicative_expression, $.exponential_expression, $.unary_expression, $.string_list_null_operator_expression, $.property_or_labels_expression, $.atom),
        or_expression: ($) => expression(0, seq($.expression, word('or'), $.expression)),
        xor_expression: ($) => expression(1, seq($.expression, word('xor'), $.expression)),
        and_expression: ($) => expression(2, seq($.expression, word('and'), $.expression)),
        not_expression: ($) => expression(3, seq(word('not'), $.expression)),
        comparison_expression: ($) => expression(4, seq($.expression, choice('=', '<>', '<', '>', '<=', '>='), $.expression)),
        addition_expression: ($) => expression(5, seq($.expression, choice('-', '+'), $.expression)),
        multiplicative_expression: ($) => expression(6, seq($.expression, choice('*', '/', '%'), $.expression)),
        exponential_expression: ($) => expression(7, seq($.expression, '^', $.expression)),
        unary_expression: ($) => expression(8, seq(choice('+', '-'), $.expression)),
        string_list_null_operator_expression: ($) => expression(9, seq($.expression, repeat1(choice($.string_operator_expression, $.list_operator_expression, $.null_operator_expression)))),
        list_operator_expression: ($) => prec.left(choice(seq(word('in'), $.expression), seq('[', $.expression, ']'), seq('[', optional($.expression), '..', optional($.expression), ']'))),
        string_operator_expression: ($) => prec.left(seq(choice(seq(word('starts'), word('with')), seq(word('ends'), word('with')), seq(word('contains'))), $.expression)),
        null_operator_expression: ($) => seq(word('is'), optional(word('not')), word('null')),
        property_or_labels_expression: ($) => expression(10, seq($.expression, choice(seq(repeat1($.property_lookup), optional($.node_labels)), seq(repeat($.property_lookup), $.node_labels)))),
        atom: ($) => expression(11, choice($.literal, $.parameter, $.case_expression, 
        ...
        function_name: ($) => choice(seq(optional($.namespace), $.symbolic_name), word('exists')),
...
function expression(precedence, rule, assoc = 'right') {
