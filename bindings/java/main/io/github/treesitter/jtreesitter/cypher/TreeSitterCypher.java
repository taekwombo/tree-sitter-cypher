package io.github.treesitter.jtreesitter.cypher;

import java.lang.foreign.*;

public final class TreeSitterCypher {
    ...
    private static final TreeSitterCypher INSTANCE = new TreeSitterCypher();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterCypher() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_cypher");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-cypher");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-cypher");
